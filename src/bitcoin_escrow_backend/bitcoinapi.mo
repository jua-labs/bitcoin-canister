...
module {
  ...
  public func get_balance(network : Network, address : BitcoinAddress) : async Satoshi {
    Debug.print("BitcoinApi: Entering get_balance function");
    Debug.print("BitcoinApi: Network: " # debug_show(network));
    Debug.print("BitcoinApi: Address: " # address);
    Debug.print("BitcoinApi: Adding cycles: " # Nat.toText(GET_BALANCE_COST_CYCLES));

    ExperimentalCycles.add(GET_BALANCE_COST_CYCLES);
    
    Debug.print("BitcoinApi: Calling management canister");
    try {
        ...
        Debug.print("BitcoinApi: Balance received: " # Nat64.toText(result));
        result
    } catch (error) {
